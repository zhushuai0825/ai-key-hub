...
const MODULE_GROUPS = [
  {
    ...
    modules: [
      ...
      { id: 'drama', name: '漫剧工作室', desc: '大纲→剧本→分镜→导出；知识库加持，对接 Local/即梦', icon: 'agent.svg', href: '/drama.html', status: 'online' },
    ],
  },
  {
    id: 'life',
...
function renderMetrics(data) {
  const card = document.getElementById('hubMetricsCard');
  const todayBox = document.getElementById('metricsToday');
  const rowsBox = document.getElementById('metricsRows');
  const meta = document.getElementById('metricsMeta');
  if (!card || !todayBox || !rowsBox) return;
  if (!data) {
    card.hidden = true;
    return;
  }
  ...
  if (meta) meta.textContent = `近 ${data.days || 14} 天 · 上海时区 · 累计登录 ${totals.logins || 0}`;
  todayBox.innerHTML = `
    <div class="hub-metrics-stat"><span>今日新增用户</span><strong>${today.new_users || 0}</strong></div>
    <div class="hub-metrics-stat"><span>今日登录</span><strong>${today.logins || 0}</strong></div>
    <div class="hub-metrics-stat"><span>今日生成剧本（集）</span><strong>${today.scripts || 0}</strong></div>
  `;
...
async function loadDashboard() {
  ...
  let metrics = null;
  [stats, providers, keys, models, usageSeries, knowledgeSummary, cacheSummary, dashboardMemory, metrics] = await Promise.all([
    api('/api/stats'),
    api('/api/providers'),
    Promise.resolve([]),
    api('/api/models'),
    ...
    api('/api/admin/metrics/daily?days=14').catch(() => null),
  ]);
  keys = [];
...
async function bindHubAuth() {
  const label = document.getElementById('hubUserLabel');
  const btn = document.getElementById('hubLogoutBtn');
  try {
    const session = await api('/api/auth/session');
    if (label && session.user) {
      label.textContent = session.user.is_admin
